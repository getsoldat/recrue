...
for /f "tokens=*" %%G in ('where git 2^>nul') do set "recrue_git_path=%%~dpG"
if not defined recrue_git_path (
  ...
  for /f "tokens=*" %%d in ('dir /b /s /a:d /od "%LOCALAPPDATA%\GitHub\PortableGit*" 2^>nul') do (
    if exist "%%d\cmd\git.exe" set "recrue_git_path=%%d\cmd"
  )
  :: Found one, add it to the path
  if defined recrue_git_path set "Path=!recrue_git_path!;!PATH!"
)
